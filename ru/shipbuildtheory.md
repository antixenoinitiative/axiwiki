...
**Базовая сборка:** Рейтинг «A» в самом большом доступном размере с модификацией «Быстрозаряжающийся распределитель питания» и экспериментальным эффектом «Сверхпроводники». The best AX weapons have extreme distributor draw, so having the maximum possible WEP capacitor regen is essential. The capacity loss of these modifications is also not a problem, since most ships can fire two of their gauss cannons at a time without spiking over ~20% heat.
